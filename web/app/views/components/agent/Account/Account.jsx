...
var UserStatus = require("../../../../models/user").User.STATUS;
    ...
    var content =
      <div>
        <LeftPanel title={this.props.title}
          activeTab={this.props.activeTab} activeClients={this.props.activeClients}/>
        <div className="container">
          <TopNavbar />
          <Profile user={user}/>
        </div>

        <ModalPopup id="actionConfirmationDialog"
...
var AccountHeader = React.createClass({
  ...
  render: function() {
    ...
    if (this.props.user.address) {
      address = <span className="icon-label">
        {(this.props.user.address.street)? this.props.user.address.street : ''}
        {(this.props.user.address.city)? this.props.user.address.city + ' ' : ''}
        {(this.props.user.address.state)? this.props.user.address.state + ' ' : ''}
        {(this.props.user.address.country)? this.props.user.address.country + ' ' : ''}
        {(this.props.user.address.zipcode)? this.props.user.address.zipcode : ''}
      </span>;
    }
    return (
      <div>
  ...
  render: function() {
    return (
      <div className="profile-picture-container">
        <img className="profile-picture" src={this.props.imgSrc}></img>
        <span className="icon icon-edit profile-picture-edit"></span>
        <span className="profile-picture-confirm">
          <span className="icon icon-check profile-picture-ok"></span>
          <span className="icon icon-cross profile-picture-cancel"></span>
        </span>
      </div>
    );
  }
...
var AccountStatusLabel = React.createClass({
  render: function () {
    return (
      <div className="account-status-label">
        <span className={"icon icon-" + this.props.icon + " " + this.props.iconClassname}></span>
...
var AccountStatusOptions = React.createClass({
  render: function () {
    return (<div className="account-status-options"><a title={this.props.title}
      className={this.props.className} data-toggle="modal"
      data-target="#actionConfirmationDialog" href="#"
      target="_blank">{this.props.label}</a></div>
    );
  }
});
...
var AccountStatus = React.createClass({
  ...
  render: function() {
    ...
    var statusLabel = 'Currently Active';
    var statusLabelIconClassname = "normal";
    var statusIcon = "check";
    var optionsTitle = "You are opting to not receive any trip planning jobs";
    var optionsClassname = "deactivate-account";
    var optionsLabel = "Deactivate your account";
    var options;
    if (status == UserStatus.ACTIVE) {
      //do nothing
    }
    else if (status == UserStatus.PENDING_APPROVAL) {
       statusLabelIconClassname = "warning";
       statusLabel = "You account is pending for approval";
       statusIcon = "warning";
       optionsTitle = "Notify support team";
       optionsClassname = "notify-support-account-approval";
       optionsLabel = "Notify support";
    }
    else if (status == UserStatus.PENDING) {
       statusLabelIconClassname = "warning";
       statusLabel = 'Pending (Please confirm your email address)';
       statusIcon = "warning";
       optionsTitle = "Resend the confirmation email";
       optionsClassname = "resend-confirmation-email";
       optionsLabel = "Resend the confirmation email";
    }
    else if (status == UserStatus.INACTIVE) {
       statusLabelIconClassname = "warning";
       statusLabel = 'Inactive (You have opted not to receive any trip planning jobs at this time)';
       statusIcon = "warning";
       optionsTitle = "You are opting to receive trip planning jobs";
       optionsClassname = "activate-account";
       optionsLabel = "Activate your account";
    }
    return (
      <div className="account-status col-sm-12">
        <AccountStatusLabel label={statusLabel} iconClassname={statusLabelIconClassname}
          icon={statusIcon}/>
        <AccountStatusOptions title={optionsTitle}
                  className={optionsClassname} label={optionsLabel}/>
      </div>
    );
  }
});
...
var Profile = React.createClass({
  ...
  render: function() {
    var user = this.props.user;
    //TBD: Add all the user fields
    var fields = [];
    for (var key in DF) {
      var fieldVals = {};
      // if ((typeof user[key]) == 'string') {
        fieldVals.pk = user._id;
        fieldVals.id = key;
        fieldVals.label = DF[key];
        fieldVals.value = user[key];
        fieldVals.editable = EF[key];
        fields.push(<Field key={key} fieldVals={fieldVals} />);
      // }
    }
    return (
      <div className="profile-form form-horizontal">
        <div className="row">
          <div className="col-sm-3">
            <AccountHeader userName={this.props.userName} user={user}/>
            <ProfilePicture imgSrc={user.image}></ProfilePicture>
          </div>
          <div className="col-sm-7">
            <label className="control-label">Profile Description</label>
            <textarea className="form-control aboutMe" rows="9" value={user.aboutMe}></textarea>
          </div>
        </div>
        <div className="row">
          <div className="col-sm-3">
            <label className="control-label">My Profile Page</label>
          </div>
          <div className="col-sm-7">
            <a href={'/profile/' + user._id} className="btn btn-default">
              {'/profile/' + user._id}
            </a>
          </div>
        </div>
        ...
        <hr className="m-t"/>
        <div>
          <ul className="nav nav-tabs" role="tablist" id="agent-profile-tab">
            <li role="presentation" className="active">
              <a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">PROFILE</a>
              </li>
            <li role="presentation">
              <a href="#office" aria-controls="office" role="tab" data-toggle="tab">OUT OF OFFICE SETTINGS</a>
            </li>
          </ul>
          <div className="tab-content">
            <div role="tabpanel" className="tab-pane active clearfix" id="profile">
              {fields}
              <div className="col-sm-8 col-sm-offset-2">
                <a href="javascript:void(0)" className="btn btn-primary-aa update-profile">
                  Save Changes
                </a>
              </div>
            </div>
            <div role="tabpanel" className="tab-pane" id="office">
              <OutOfficeField user={user} />
            </div>
          </div>
        </div>
...
var Field = React.createClass({
  ...
  render: function() {
    const fieldVals = this.props.fieldVals;
    let fieldEle = null;
    //set empty value for non existing fields
    if (!fieldVals || !fieldVals.value) {
      fieldVals.value = "";
    }
    if (["destinations", "specialities"].indexOf(fieldVals.id) !== -1) {
      fieldVals["value"] = fieldVals.value.split(";");
      let itemNode = "";
      if (fieldVals.value.length > 0) {
        itemNode = fieldVals.value.map(function(item, i) {
          if (item === "") { return;}
          return (
            <p className="label label-success item">
              <span className='item-val'>{item}</span>
            </p>
          );
        })
      }
      fieldEle = <div className="col-sm-8">
                  <div className="panel panel-default">
                    <div className="panel-body">
                      <div className={`${fieldVals.id}-items clearfix`}>
                        {itemNode}
                      </div>
                      <div className="form-inline">
                        <div className="form-group">
                          <input type="text" data-field={fieldVals.id} className={`${fieldVals.id} form-control add-input`} placeholder="" />
                          <a href="javascript:void(0)" className="add-items btn btn-default" data-field={fieldVals.id}>
                            {/*<span className="ti-plus"></span>*/} Add
                          </a>
                        </div>
                      </div>
                    </div>
      ...
      let fieldVal = fieldVals.value;
      if (fieldVals.id === 'phone') {
        fieldVal = fieldVal.mobile;
      }
      fieldEle =
        <div className="col-sm-8">
          <input type="text" disabled={fieldVals.id === 'email' ? true : false} className={`${fieldVals.id} form-control`} ref={fieldVals.id} placeholder="" value={fieldVal} />
        </div>
    }
    return (
      <div className="form-group">
        <label className="category-1 field-name col-sm-2 control-label">{fieldVals.label}:</label>
        {/*<label className="category-1 field-value">{this.props.fieldVals.value}</label>*/}
        {fieldEle}
      </div>
    );
  }
...
var OutOfficeField = React.createClass({
  getInitialState: function () {
    return {};
  },
  render: function() {
    ...
    return (
      <div className="clearfix">
        <div className="form-group">
          ...
          <div className="col-sm-8">
            <textarea className="form-control message" rows="6" value={user.availability ? user.availability.oooMessage : ''}></textarea>
          </div>
        </div>
        <div className="form-group">
          <label className="col-sm-2 control-label">Timezone:</label>
          <div className="col-sm-8">
            <select className="form-control timezone" value={user.timeZone ? user.timeZone : "America/Tijuana"} name="DropDownTimezone" id="DropDownTimezone">
              ...
              <option selected="" value="America/Los_Angeles">(GMT-8:00) America/Los_Angeles (Pacific Standard Time)</option>
        ...
        <table className="table">
          <tbody>
            <tr>
              <th><label className="control-label">Availability:</label></th>
              {weekday.map(function(item, i) {
                let isChecked = true;
                if (["Saturday", "Sunday"].indexOf(item) !== -1) {
                  isChecked = false;
                }
                return (
                  <td>
                    <label for={`weekday-${item}`} className="checkbox-inline">
                      <input checked={isChecked} type="checkbox" id={`weekday-${item}`} className="weekday" value={item} />{item}
                    </label>
                  </td>
                )
              })}
            ...
            <tr>
              <th><label className="control-label">Start Time:</label></th>
              {weekday.map(function(item, i) {
                let start = user.availability[item].start;
                return (
                  <td>
                    <select className={`form-control start-time ${item}`} value={start ? start : "08:00 am"} name="" id="">
                      ...
                      <option value="12:00 pm">00:00 pm</option>
                      <option value="12:30 pm">00:30 pm</option>
            ...
            <tr>
              <th><label className="control-label">End Time:</label></th>
              {weekday.map(function(item, i) {
                let end = user.availability[item].end;
                return (
                  <td>
                    <select className={`form-control end-time ${item}`} value={end ? end : "05:00 pm"} name="" id="">
                      <option value="00:00 am">00:00 am</option>
                      <option value="00:30 am">00:30 am</option>
                      <option value="01:00 am">01:00 am</option>
                      <option value="01:30 am">01:30 am</option>
        ...
        <div className="col-sm-8 col-sm-offset-2">
          <a href="javascript:void(0)" className="btn btn-primary-aa save-settings">
            Save Settings
          </a>&nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" className="btn btn-primary-aa disable-settings">
            Disable Settings
          </a>
        </div>
      </div>
