$( document ).ready(function() {
	...
	$('#agentDetailsDialog').on('show.bs.modal', function (event) {
	  ...
	  var modal = $(this);
        ...
        // Set up FBAgentMatcher.
        (function () {
          const MESSAGE_SUCCESS = 'Successfully assigned agent to a FB chat';
          const MESSAGE_ERROR = 'Failed to assign agent to a FB chat';

          function assignAgent(tripId, agentId, agentEmail, success) {
            $.ajax({
              url: '/trip/' + tripId,
              type: 'POST',
              dataType: 'json',
          ...
          $('.FBAgentMatcher-chat-agentButton').on('click', function (event) {
            event.preventDefault();
            const $this = $(this);
            assignAgent(
              $this.attr('data-trip-id'),
              $this.attr('data-agent-id'),
              $this.attr('data-agent-email'),
              function () {
                location.reload();
              }
            );
